(function ($) {
    ...
    var flipTimer = function (element, options) {
        ...
        // detects if the data digits should be used
        if (this.element.find('.data').length > 0) {
            this.options.data = this.element.find('.data')[0];
        }
    ...
    flipTimer.prototype = {
        ...
        calculateDate: function () {
            ...
            this.endVal = this.options.endVal;
        ...
        render: function () {
            ...
            // if using startVal, populate it
            if (this.options.startVal) {
                this.renderDigits(this.options.data, this.startVal);
            }
        ...
        renderDigits: function (subject, value) {
            ...
            if ($(subject).find('.digit').length == 0) {
                ...
                if (_this.days < 0 && _this.hours < 0 && _this.minutes < 0 && _this.seconds < 0 && _this.userOptions.type == 'date' ) {
                    number_array = [0, 0];
                } else if (_this.days > 99 && _this.userOptions.type == 'date') {
                    number_array = [0, 0];
                } else if (_this.userOptions.type == 'date') {
                    number_array = String((value / 10).toFixed(1)).split('.');
                } else if (_this.userOptions.type == 'number') {
                ...
                if(_this.options.type == 'date'){
                    $(subject).append('<div class="digit-set"></div><div class="digit-set"></div>');
                } else if (_this.options.type == 'number'){
                    for ( var j = 0; j < _this.startVal.length; j++ ) {
                        $(subject).append('<div class="digit-set"></div>');
                    }
                }

                $(subject).find('.digit-set').css({
                    height : _this.options.height,
                    width : _this.options.width,
                    color : _this.options.fontColor,
        ...
        startTimer: function () {
            ...
            this.timer = setInterval(function () {
                if(_this.options.type=='date') {
                    ...
                    if (_this.days <= 0 && _this.hours <= 0 && _this.minutes <= 0 && _this.seconds <= 0 && _this.options.type == 'date') {
                    ...
                    // if timer runs out stop the timer
                    if ((_this.days > 99) || (_this.days == 99 && _this.hours == 23 && _this.minutes == 59 && _this.seconds == 59 && _this.options.type == 'date')) {
                        clearInterval(_this.timer);
                        return;
                    }

                    // increase/decrease seconds
                    (_this.options.direction == 'down' && _this.options.type == 'date') ? _this.seconds-- : _this.seconds++;
                    if (_this.options.seconds) _this.increaseDigit(_this.options.seconds);

                    // increase/decrease minutes
                    if ((_this.seconds == 60 || _this.seconds == -1) && _this.options.type == 'date') {
                    ...
                    if ((_this.minutes == 60 || _this.minutes == -1) && _this.options.type == 'date') {
                        if (_this.options.direction == 'down') {
                            _this.minutes = 59;
                            _this.hours--;
                        } else {
                    ...
                    if ((_this.hours == 24 || _this.hours == -1) && _this.options.type == 'date') {
                        if (_this.options.direction == 'down') {
                            _this.hours = 23;
                            _this.days--;
                        } else {
                    ...
                    if(_this.options.direction == 'down'){
                        if(_this.startVal - _this.endVal > 0 ) {
                            _this.startVal--;
                            _this.increaseDigit(_this.options.data);
                        }
                    }else{
                        if(_this.startVal - _this.endVal < 0 ) {
                            _this.startVal++;
    ...
    $.fn.flipTimer = function (options) {
        ...
        return this.each(function () {
            if (!$(this).data('flipTimer')) {
                console.log(!$(this).data('flipTimer'));
